...
```mermaid
flowchart LR
  INPUT[3 исходных Parquet] --> V[Проверка схем, сумм, количества и концов рёбер]
  C[config.toml] --> F
  V --> G[Полный направленный граф с изолятами]
  G --> F[Потоки, seed, структура, дневные признаки]
  G --> PATH[Кратчайший направленный путь от seed]
...
Первым уже выбран новый участник. Переключить левую область на «Общие узлы» и ввести исходные gid
...
В области связей выбрать по очереди маршруты от каждого источника: 2 и 4 шага.
Они открываются на основном графе с хронологией и выделением конфликта дат.
...
и оборот дают соответственно 26,3, 34,9 и 20,9 балла из итоговых 82,1/100.
Рядом видны пороги правил из конфигурации расчёта.
...
или следующие два дня 75%, альтернативные роли и агрегированные связи с контрагентами за месяц.
...
Отметить «Включить в проверку», добавить заметку и выгрузить список проверки
через «Выгрузки» сверху. Показать восстановление выбранного узла и маршрута
после перезагрузки. Общие файлы доступны там же: «Участники и роли»,
«Кластеры», «Приоритеты проверки» (nodes_roles.csv, clusters.csv, top_nodes.csv). Везде числовые
