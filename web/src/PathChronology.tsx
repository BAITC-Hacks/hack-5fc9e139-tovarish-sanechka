  ...
  for (const strict of [true, false]) {
    ...
    if (!strict)
      return {
        status: "inconsistent",
        dates,
        failedStep: dates.length,
      } as const;
  }
  throw new Error("Не удалось проверить хронологию пути");
}

export function PathChronology({
  edges,
  onSelect,
}: {
  edges: Edge[];
  ...
  const status =
    edges.length === 1
      ? "Один шаг: дата операции указана ниже"
  ...
  return (
    <div className="path-chronology">
      <p className="chronology-status">
        <strong>{status}</strong>
      </p>
      {failed && (
        <p className="muted">
          Последовательность прерывается на шаге {result.failedStep! + 1}:{" "}
          {failed.src} → {failed.dst}. Доступные даты: {failed.dates.join(", ")}
          . Другой маршрут может существовать.
        </p>
      )}
      <ol>
        {edges.map((edge, index) => (
          <li
            key={edge.id}
            className={index === result.failedStep ? "conflict-step" : ""}
          >
            <span className="path-step">
              <button
                className="link-button"
                onClick={() => onSelect(edge.src)}
              >
                {edge.src}
              </button>
              ...
              <button
                className="link-button"
                onClick={() => onSelect(edge.dst)}
              >
                {edge.dst}
              </button>
            </span>
            <span>
              {number(edge.sum_kzt)} ₸ · {edge.n_tx} операций за июль
            </span>
            <small>
              {result.dates[index]
                ? `Дата операции: ${result.dates[index]}`
                : `Даты операций: ${edge.dates.join(", ")}`}
            </small>
          </li>
        ))}
      </ol>
      {edges.length > 1 && (
        <p className="muted">
          Путь не доказывает движение одних и тех же денег. Суммы связей — за
          весь месяц.
        </p>
      )}
    </div>
