...
function pathData(base: Analysis, dates: string[][]): Analysis {
  const nodes = ids.map((gid, index) => ({
    ...
    gid,
    is_seed: index < 2,
    depth: Math.max(0, index - 1),
    cluster_id: 0,
    priority_score: (4 - index) / 4,
    reachable_seed_count: index < 2 ? 0 : 2,
    seed_path_gids:
      index < 2 ? [gid] : index === 2 ? [ids[0], gid] : [ids[0], ids[2], gid],
  }));
  const pairs = [
    [ids[0], ids[2]],
    [ids[1], ids[2]],
  ...
  return {
    ...
    meta: {
      ...
      n_nodes: 4,
      n_edges: 3,
      n_transactions: edges.reduce((sum, e) => sum + e.n_tx, 0),
    },
    top_nodes: nodes.map((node, index) => ({
      ...node,
      rank: index + 1,
      why: node.evidence,
    })),
    clusters: [
      {
        ...
        top_gids: ids,
        hypothesis: "Тестовая сеть",
      },
    ],
    resilience: {
      ...base.resilience,
      baseline_largest_component: 4,
      scenarios: [],
    },
  };
}
...
const cases = [
  {
    ...
    status: "ordered",
    text: "Последовательность дат согласована",
    chosen: ["2026-07-01", "2026-07-02"],
  },
  {
    name: "same day",
    dates: [["2026-07-02"], ["2026-07-02"]],
    status: "same_day",
    text: "Порядок внутри дня неизвестен",
    chosen: ["2026-07-02", "2026-07-02"],
  },
  {
    name: "backwards",
    dates: [["2026-07-03"], ["2026-07-02"]],
    status: "inconsistent",
    text: "Выбранный путь не согласован по датам",
    chosen: ["2026-07-03"],
  },
  {
    name: "strict alternative to same day",
    dates: [["2026-07-01"], ["2026-07-01", "2026-07-03"]],
    status: "ordered",
    text: "Последовательность дат согласована",
    chosen: ["2026-07-01", "2026-07-03"],
  },
];

for (const scenario of cases) {
  test(`chronology: ${scenario.name} in both path views`, async ({
    page,
    request,
  }) => {
    ...
    const result = checkPathChronology(
      scenario.dates.map((dates) => ({ dates })),
    );
    ...
    await expect(page.locator(".path-details .chronology-status")).toHaveText(
      scenario.text,
    );
    for (const [index, day] of scenario.chosen.entries())
      await expect(page.locator(".path-details li").nth(index)).toContainText(
        `Дата операции: ${day}`,
      );
    if (scenario.status === "inconsistent") {
      await expect(page.locator(".path-details")).toContainText(
        "прерывается на шаге 2",
      );
      await expect(page.locator(".path-details")).toContainText(
        "Доступные даты: 2026-07-02",
      );
    }
    ...
    for (let index = 0; index < 2; index++) {
      ...
      if (scenario.status === "inconsistent") {
        await expect(page.getByRole("img")).toHaveAttribute(
          "aria-label",
          /Конфликт дат выделен/,
        );
        await expect(page.locator(".conflict-step")).toContainText(
          "2026-07-02",
        );
      }
    }
    ...
    if (scenario.status === "inconsistent") {
      await page.screenshot({
        path: "/tmp/hackalem-chronology-mobile.png",
        fullPage: true,
      });
      await page.setViewportSize({ width: 1440, height: 1000 });
      await page.screenshot({
        path: "/tmp/hackalem-chronology-desktop.png",
        fullPage: true,
      });
    }
  });
...
test("chronology handles a single edge, an empty path and invalid dates", async ({
  page,
  request,
}) => {
  const base: Analysis = await (await request.get("/analysis.json")).json();
  ...
  for (const dates of [
    ...
    expect(() =>
      validateAnalysis({
        ...data,
        edges: [{ ...data.edges[0], dates }, ...data.edges.slice(1)],
      }),
    ).toThrow("повреждён");
  }
  expect(() => validateAnalysis({ ...data, schema_version: 1 })).toThrow(
    "повреждён",
  );
  await page.route("**/analysis.json", (route) =>
    route.fulfill({ json: data }),
  ...
  await page.goto("/");
  await page.getByLabel("Найти узел по gid").fill(ids[0]);
  await page.getByLabel("Найти узел по gid").press("Enter");
  await page.getByLabel("Область графа").selectOption("path");
  await expect(page.locator(".path-details .chronology-status")).toHaveCount(0);
  await page.getByLabel("Найти узел по gid").fill(ids[2]);
  await page.getByLabel("Найти узел по gid").press("Enter");
  await expect(page.locator(".path-details .chronology-status")).toContainText(
    "Один шаг",
  );
  await expect(page.locator(".path-details li")).toContainText(
