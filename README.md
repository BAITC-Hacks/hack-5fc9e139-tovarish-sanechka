...
Быстрый запуск из корня репозитория: `docker compose up -d --build --wait --wait-timeout 360`, затем открыть **http://localhost:8080**. Для основных функций `.env` и API-ключ не нужны; первая сборка требует сети. Готовые CSV и схема решения лежат в [submission/](submission/), сценарий защиты — в [DEMO.md](docs/DEMO.md).
